...
- 存在对外交付副本 `delivery/cdid-analysis-platform/`，与核心目录并行存在。
- 根目录混有历史项目与规格文档（`data-insight-service/`、`specs_v0_1/`、`xuqiu/`），已执行归档迁移。
- 根目录存在代理输出与补丁文件，已迁移到 `artifacts/agent_outputs/`。
- 现有工作区包含你之前的业务代码修改（`cdid-analysis-platform/` 与 `delivery/` 下多文件改动），本次未覆盖这些改动。
...
| `delivery/cdid-analysis-platform/` | 对外交付副本 | 保留原位（后续可独立治理） | 已保留 | 当前有并行修改，不在本阶段做大迁移 |
...
| `delivery.zip` | 历史压缩包 | 已删除状态，待确认是否恢复 | 待确认 | 当前 git 显示删除，需要确认交付策略 |
